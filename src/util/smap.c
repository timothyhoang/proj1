...
void smap_print_strings(smap *map) {
    printf(".data\n");
    int counter = 0;
    for (size_t i = 0; i < map->num_buckets; i += 1) {
	for (size_t j = 0; j < map->buckets[i].num_pairs; j += 1) {
	    printf("string");
	    printf("%d: .asciiz \"", counter);
	    printf("%s", map->buckets[i].pairs[j].key);
	    map->buckets[i].pairs[j].val = counter;
	    printf("\"\n");
	    counter++;
	}
    }
...
void smap_print_static_vars(smap *map) {
    printf(".data\n");
    for (size_t i = 0; i < map->num_buckets; i += 1) {
	for (size_t j = 0; j < map->buckets[i].num_pairs; j += 1) {
	    if(map->buckets[i].pairs[j].val == 0)
		printf("%s: .word 0:1\n", map->buckets[i].pairs[j].key);
	}
    }
    printf("\n");
}

void smap_print_static_structs(smap *map) {
    printf(".data\n");
    int counter = 0;
    for (size_t i = 0; i < map->num_buckets; i += 1) {
	for (size_t j = 0; j < map->buckets[i].num_pairs; j += 1) {
	    printf("struct");
	    printf("%d: .word 0:%d\n", counter, map->buckets[i].pairs[j].val);
	    map->buckets[i].pairs[j].val = counter;
	    counter++;
	}
    }
